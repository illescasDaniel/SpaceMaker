...
## 2026-09-24 — Native pywebview backend on Windows/macOS; Qt WebEngine kept Linux-only

- **Context:** User asked why Qt was a dependency at all given pywebview is already the desktop shell, and whether it could be dropped to cut app size/overhead. Assessed: `--gui` defaulted to `qt` unconditionally on every OS, and `pyqt6`/`pyqt6-webengine`/`qtpy` were unconditional `pyproject.toml` dependencies, even though pywebview has native, dependency-free backends on Windows (`edgechromium`, WebView2, ships with Windows 10/11) and macOS (`cocoa`, WKWebView, built into the OS). Linux is the one platform where dropping Qt has a real cost: the Linux AppImage deliberately bundles Qt WebEngine (see the "Linux 1.0 release" entry below) specifically to pin a known Chromium version instead of depending on the host distro's `webkit2gtk`, so switching Linux to the `gtk` backend would reopen that exact problem.
- **Decision:** `desktop.py` gets `_default_gui_backend()`: `edgechromium` on `win32`, `cocoa` on `darwin`, `qt` otherwise (Linux); `--gui` still overrides for dev/testing. All Qt-only setup (`install_qt_webengine_gpu_flags`, `install_qt_webengine_shutdown_fix`, `install_qt_native_style`, `_apply_qt_window_icon`, `finalize_qt_after_webview`) is now gated on the resolved backend actually being `"qt"`, not just called unconditionally. `pyproject.toml` marks `pyqt6`, `pyqt6-webengine`, `qtpy` as `sys_platform == 'linux'`-only. `packaging/spacemaker.spec` (Windows/macOS PyInstaller onefile) now excludes whichever pywebview backends the build OS doesn't use (including `qt` itself on Windows/macOS), computed from `sys.platform` at spec-build time. Linux AppImage packaging (`prune_pyqt6.sh`, `build-appdir.sh`) is untouched. Added `tests/unit/test_desktop_gui_backend.py`.
- **Rationale:** Windows/macOS get a strictly smaller, simpler dependency set (no PyQt6/PyQt6-WebEngine wheels, no Qt/Chromium-compositor shutdown-teardown or GPU-flag workarounds to maintain) with no downside — WebView2/WKWebView are the natural choice on those OSes regardless of app size. Linux keeps Qt because the AppImage's whole point is a self-contained, version-pinned Chromium; trading that for a `webkit2gtk` dependency would just move the "which engine version do we get" problem onto the user's distro, which is what the existing AppImage decision was made to avoid. Not verified with a real Windows/macOS GUI session (no such hardware in this sandbox) — only import/dependency-resolution logic and the backend-selection function were tested; a real click-through on Windows/macOS is still pending, same caveat as the earlier Qt GPU-flag fix.

## 2026-09-24 — `QDxgiVSyncService`/`QThreadStorage` shutdown warnings: narrowed the earlier "not pursuing further" decision to a log filter, since the actual ask was noise, not the race

- **Context:** User re-reported the same two lines from the entry below. Confirmed on questioning: still just console noise on exit, no hang/crash/non-zero exit — i.e. exactly the "otherwise leave `qt_webengine_shutdown.py` as-is" branch of that decision. The narrower ask was to stop the lines from printing, not to chase the underlying async DXGI-teardown race further.
- **Decision:** Did **not** touch `finalize_qt_after_webview()`, `finish_quit()`, or any of the existing drain-round/sleep timing — that logic is confirmed to already prevent real hangs/crashes and tuning it further was already ruled out as not worth it. Instead added `_is_benign_shutdown_warning()` and `install_benign_shutdown_warning_filter()` to `qt_webengine_shutdown.py`: a `QtCore.qInstallMessageHandler` wrapper that drops only messages starting with `"QDxgiVSyncService not destroyed in time"` or `"QThreadStorage: entry"`, forwarding everything else (including other warnings) to whatever handler was previously installed. Wired in from `install_qt_webengine_shutdown_fix()`, active for the app's full lifetime (no need to uninstall — the process ends via `os._exit(0)` shortly after). Confirmed via `qtpy`/PySide6 that `qInstallMessageHandler(None)` returns the previously-installed handler (or `None` for Qt's default), which the wrapper falls back to a plain `sys.stderr.write` for. User confirmed on a real run that the window still closes promptly and the two lines no longer appear.
- **Rationale:** Both messages are genuine Qt (qtbase) internal `qWarning()`-style log lines — not raw Chromium subprocess stderr — so `qInstallMessageHandler` is the correct, minimal-risk mechanism (as opposed to wrapping `sys.stderr`, which would also risk swallowing output from an unrelated child process). Matching by message-text prefix rather than log level/category keeps the filter narrow: only these two known-benign lines are suppressed, every other Qt warning still reaches the console exactly as before.

## 2026-09-24 — Lean agent context: `AGENTS.md` sole always-on source, path-scoped rules, memory archive

- **Context:** The user asked to review the full loaded agent context (`AGENTS.md`, `.cursor`/`.claude` rules and skills, `memory/`) and reduce dilution/duplication. Measured before this change: `AGENTS.md` (14.4KB) plus five always-on `.claude/rules/*.md` (7.5KB) loaded every session — ~22KB before any work — on top of `memory/activeContext.md` (9.9KB) and `memory/progress.md` (6.4KB) read at session start. The same guidance was stated in 3–5 places each: phase gates in `AGENTS.md`, the `sdd` rule, the `sdd-feature` skill, and the `playbooks` rule/skill; the memory protocol in `AGENTS.md`, the `agent-memory` rule, the `agent-memory` skill, and `memory/README.md`; hexagonal layering in `AGENTS.md`, the `hexagonal-python` rule, and the `hexagonal-python` skill. `progress.md` also had stale entries (`docs/database.md`, which had been deleted; an "Open" header holding mostly `[x]` items).
- **Decision:** `AGENTS.md` is now the single always-on instructions file (both Cursor and Claude Code read it natively) — cut from 14.4KB to ~6KB, keeping only the hard rules, the compact phase-gate table, the memory-bank triggers, and a "where to look" index, with detail moved out. The four rule pairs that mostly repeated `AGENTS.md` (`agent-memory`, `sdd`, `playbooks`, `graphrag-tools`) were deleted from both `.cursor/rules/` and `.claude/rules/`; their content now lives in `AGENTS.md`, the `agent-memory`/`sdd-feature` skills, or the new `docs/agent-tooling.md` (Graphify advanced commands, rule-pairing maintenance notes, durable Windows/tooling gotchas). Only `hexagonal-python` remains a rule, rewritten as **path-scoped** (`globs`/`paths`: `src/spacemaker/**`) instead of always-on, absorbing the now-deleted `hexagonal-python` skill's placement table. `.cursor/skills/playbooks/` and `.cursor/skills/hexagonal-python/` were deleted as redundant with `AGENTS.md` and the rule, respectively. New `tests/unit/test_agent_context.py` runs in `uv run task checks` and asserts: every `.cursor/rules/*.mdc` has a `.claude/rules/*.md` counterpart (and vice versa) with byte-identical bodies after frontmatter and equivalent scoping (`alwaysApply: true` ⇔ no `paths`; `alwaysApply: false` + `globs` ⇔ matching `paths`); no Cursor-only rule kinds (description-only "agent requested", manual) are used, since Claude Code has no equivalent; `AGENTS.md` stays under a 130-line budget; and no tracked context file references the deleted rule/skill paths. `memory/progress.md` was cut to open items only; finished Done lists and old `activeContext.md` threads moved to new `memory/archive.md` (not read at session start, referenced from `memory/README.md`).
- **Rationale:** Repeated, lengthy guidance dilutes the rules that matter most (phase gates, the hexagonal import guard) and risks silent drift between the Cursor and Claude Code copies, since nothing previously checked they matched. The user explicitly requires Cursor and Claude Code to receive **equivalent** guidance — this is now enforced by a test rather than relying on remembering to hand-edit both files, and is also saved to Claude's cross-session memory so future sessions keep new rule pairs equivalent by construction. Loading only the essentials every session, with the rest one skill/doc-read away, keeps agent behavior sharp without losing any information — it's relocated, not deleted.

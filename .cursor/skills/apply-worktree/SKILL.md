...
2. **Commit worktree changes first (required if dirty)**
   ...
   - Follow the user’s git safety rules and commit style: review `git status` / `git diff` / recent `git log`; stage relevant paths only (never secrets such as `.env` or credential files); no `--no-verify`, no amend unless the usual amend conditions are met, no force-push, no git config changes, no `-i`.
...
5. **Quality gate (required)**
   ...
   - This runs **ruff**, **ty**, **pytest**, and **Biome** (web static assets). See [AGENTS.md](../../AGENTS.md) and [scripts/quality/checks.sh](../../scripts/quality/checks.sh).
